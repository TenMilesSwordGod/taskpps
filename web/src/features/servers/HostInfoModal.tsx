...
function SectionHeader({
  ...
  return (
    ...
    >
      <span style={{ display: 'inline-flex', color: '#8C8C8C' }}>{icon}</span>
      <span style={{ fontSize: 12, fontWeight: 600, color: '#262626', letterSpacing: 0.3 }}>{label}</span>
...
function InfoRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, padding: '4px 0', fontSize: 12 }}>
      <span style={{ color: '#8C8C8C', width: 72, flexShrink: 0 }}>{label}</span>
      <span style={{ color: '#262626', fontFamily: 'JetBrains Mono, monospace', wordBreak: 'break-word', minWidth: 0 }}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '5px 0' }}>
      <span
        title={disk.mount}
        style={{
          fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: '#262626',
      ...
      {/* 使用率细条 */}
      <div style={{ flex: 1, height: 6, background: '#F5F5F5', borderRadius: 3, overflow: 'hidden', minWidth: 60 }}>
        <div
          style={{
      ...
      <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: '#8C8C8C', flexShrink: 0, textAlign: 'right' }}>
        {disk.used} / {disk.size}
      </span>
    ...
    >
      ...
      {data && !isLoading && (
        <>
          ...
          />
          <div style={{ fontSize: 12, color: '#262626', fontFamily: 'JetBrains Mono, monospace', wordBreak: 'break-word', padding: '4px 0' }}>
            {data.cpu.model || '—'}
          </div>
          ...
          {data.memory.percent >= 0 ? (
            <div style={{ padding: '4px 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#8C8C8C', marginTop: 4, fontFamily: 'JetBrains Mono, monospace' }}>
            ...
            <div style={{ height: 6, background: '#F5F5F5', borderRadius: 3, margin: '4px 0' }} />
          )}
