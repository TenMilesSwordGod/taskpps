...
**触发机制：** 鼠标悬浮 300ms 延迟后弹出浮窗（避免误触），移出浮窗区域关闭；触摸设备上 tap 切换显隐。
...
- 当前项目显示选中态
- 点击其他项目 → 跳转到 `/pipelines/{newProjectId}`，列表页自动选中/过滤到该项目，只展示该项目的流水线
- 点击外部 / 移出浮窗 → 关闭
...
**加载策略：** 浮窗数据（项目列表 / 当前项目流水线列表）均为**悬浮时按需加载**，不随页面初始化一并拉取，减少不必要的请求。
...
// 获取项目列表（悬浮时才触发请求，用于浮窗内容）
const { data: projects, refetch: refetchProjects } = useProjects();
// 实际使用：onPopoverOpen → refetchProjects() 或直接用 enabled 控制

// 获取当前项目所有流水线（悬浮时才触发请求，用于浮窗内容）
const { data: projectPipelines, refetch: refetchPipelines } = usePipelines({ project_id: projectId });
// 实际使用：onPopoverOpen → refetchPipelines() 或直接用 enabled 控制
```

**列表分批展示：** 由于后端不返回分页，前端拉取全量数据后做客户端分批：
- 首次展示 20 条
- 滚动到底部时追加 20 条
- 搜索过滤时在所有已加载数据中过滤
...
**建议行为：** 跳转到 `/pipelines/{newProjectId}`，列表页自动选中/过滤到该项目，只展示该项目的流水线。
...
| 浮窗弹出 | 延迟 300ms 弹出，150ms 淡入动画；移出浮窗 / 点击外部 / Esc 关闭 |
| 浮窗最大高度 | 320px（项目列表）/ 400px（流水线列表），超出滚动 |
| 搜索 | 浮窗内项目/流水线列表支持即时搜索过滤 |
| 选中态 | 左侧蓝点/高亮背景 |
| 列表容量 | 默认展示前 20 条；滚动到底部时自动追加后续数据（前端分批加载，每次 20 条），直至全部展示完毕 |
