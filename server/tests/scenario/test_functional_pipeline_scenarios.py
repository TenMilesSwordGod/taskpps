...
class TestRealWorldPipelineScenarios:
    ...
    async def test_ci_cd_full_pipeline(self, db_engine, clean_db):
        ...
        tasks = [
            ...
            ResolvedTask(name="integration-test", task_type="command", command="echo integration tests...", depends_on=["build"]),
            ResolvedTask(name="deploy-staging", task_type="command", command="echo deploying staging...", depends_on=["unit-test", "integration-test"]),
            ResolvedTask(name="smoke-test", task_type="command", command="echo smoke testing...", depends_on=["deploy-staging"]),
            ResolvedTask(name="deploy-prod", task_type="command", command="echo deploying prod...", depends_on=["smoke-test"]),
        ]
    ...
    async def test_microservice_deploy_with_shared_dependency(self, db_engine, clean_db):
        ...
        tasks = [
            ...
            ResolvedTask(name="deploy-auth", task_type="command", command="echo deploying auth...", depends_on=["build-lib"]),
            ResolvedTask(name="deploy-api", task_type="command", command="echo deploying api...", depends_on=["build-lib"]),
            ResolvedTask(name="deploy-gateway", task_type="command", command="echo deploying gateway...", depends_on=["build-lib"]),
            ResolvedTask(name="health-check", task_type="command", command="echo health check...",
    ...
    async def test_many_independent_tasks_all_execute(self, db_engine, clean_db):
        ...
        _setup_config()
        tasks = [
            ResolvedTask(name=f"task-{i:02d}", task_type="command", command=f"echo {i}")
            for i in range(20)
        ]
        sub = ResolvedSubPipeline(
            name="many",
    ...
    async def test_deep_dependency_chain(self, db_engine, clean_db):
        ...
        for i in range(9):
            assert execution_order.index(f"t{i}") < execution_order.index(
                f"t{i+1}"
            ), f"t{i} 应在 t{i+1} 之前"
    ...
    async def test_mixed_strategy_subpipelines_parallel_then_sequential(self, db_engine, clean_db):
        ...
        pipeline = ResolvedPipeline(
            name="p", subpipelines=[sub_deploy, sub_verify], top_config=PipelineConfig()
        )
        ...
        runner._task_run_ids = {
            "deploy.svc-a": "tr-a", "deploy.svc-b": "tr-b", "deploy.svc-c": "tr-c",
            "verify.check-a": "tr-ka", "verify.check-b": "tr-kb",
        }
    ...
    async def test_cancel_during_parallel_execution(self, db_engine, clean_db):
        ...
        _setup_config()
        tasks = [
            ResolvedTask(name=f"t{i}", task_type="command", command=f"echo {i}")
            for i in range(5)
        ]
        sub = ResolvedSubPipeline(
            name="sub",
