...
class TestBuildEnv:
    @pytest.mark.zentao("TC-S1034", domain="server/domain", priority="P2")
    def test_empty(self):
        result = build_env()
        assert result == {}

    @pytest.mark.zentao("TC-S1035", domain="server/domain", priority="P1")
    def test_system_env(self):
        result = build_env(system_env={"PATH": "/usr/bin"})
        assert result["PATH"] == "/usr/bin"

    @pytest.mark.zentao("TC-S1036", domain="server/domain", priority="P1")
    def test_global_env(self):
        result = build_env(global_env={"GLOBAL": "val"})
        assert result["GLOBAL"] == "val"

    @pytest.mark.zentao("TC-S1037", domain="server/domain", priority="P1")
    def test_pipeline_env(self):
        result = build_env(pipeline_env={"PIPE": "val"})
        assert result["PIPE"] == "val"

    @pytest.mark.zentao("TC-S1038", domain="server/domain", priority="P1")
    def test_task_env(self):
        result = build_env(task_env={"TASK": "val"})
        assert result["TASK"] == "val"

    @pytest.mark.zentao("TC-S1039", domain="server/domain", priority="P1")
    def test_cli_env(self):
        result = build_env(cli_env={"CLI": "val"})
        assert result["CLI"] == "val"

    @pytest.mark.zentao("TC-S1040", domain="server/domain", priority="P2")
    ...
    @pytest.mark.zentao("TC-S1041", domain="server/domain", priority="P2")
...
class TestExecutionContext:
    @pytest.mark.zentao("TC-S1042", domain="server/domain", priority="P2")
    ...
    @pytest.mark.zentao("TC-S1043", domain="server/domain", priority="P1")
    def test_with_env(self):
        pipeline = _make_pipeline("test")
        ctx = ExecutionContext(pipeline=pipeline, run_id="run-1", env={"KEY": "VAL"})
        assert ctx.env == {"KEY": "VAL"}

    @pytest.mark.zentao("TC-S1044", domain="server/domain", priority="P2")
    ...
    @pytest.mark.zentao("TC-S1045", domain="server/domain", priority="P2")
    def test_get_workspace_nonexistent(self):
        pipeline = _make_pipeline("test")
        ctx = ExecutionContext(pipeline=pipeline, run_id="run-1")
        assert ctx.get_workspace("nonexistent") is None

    @pytest.mark.zentao("TC-S1046", domain="server/domain", priority="P2")
    ...
    @pytest.mark.zentao("TC-S1047", domain="server/domain", priority="P2")
    def test_get_workspace_no_args_empty(self):
        pipeline = _make_pipeline("test")
        ctx = ExecutionContext(pipeline=pipeline, run_id="run-1")
        assert ctx.get_workspace() is None

    @pytest.mark.zentao("TC-S1048", domain="server/domain", priority="P1")
...
class TestResolveDotPath:
    @pytest.mark.zentao("TC-S1049", domain="server/domain", priority="P2")
    def test_simple_key(self):
        data = {"key": "value"}
        assert resolve_dot_path(data, "key") == "value"

    @pytest.mark.zentao("TC-S1050", domain="server/domain", priority="P2")
    def test_nested_key(self):
        data = {"a": {"b": "c"}}
        assert resolve_dot_path(data, "a.b") == "c"

    @pytest.mark.zentao("TC-S1051", domain="server/domain", priority="P2")
    def test_deep_nested(self):
        data = {"a": {"b": {"c": {"d": "value"}}}}
        assert resolve_dot_path(data, "a.b.c.d") == "value"

    @pytest.mark.zentao("TC-S1052", domain="server/domain", priority="P2")
    def test_list_index(self):
        data = {"items": ["a", "b", "c"]}
        assert resolve_dot_path(data, "items.1") == "b"

    @pytest.mark.zentao("TC-S1053", domain="server/domain", priority="P1")
...
class TestApplyOverrides:
    @pytest.mark.zentao("TC-S1054", domain="server/domain", priority="P2")
    ...
    @pytest.mark.zentao("TC-S1055", domain="server/domain", priority="P2")
    ...
    @pytest.mark.zentao("TC-S1056", domain="server/domain", priority="P1")
    ...
    @pytest.mark.zentao("TC-S1057", domain="server/domain", priority="P2")
    def test_invalid_override_path(self):
        data = {"name": "test", "tasks": []}
        with pytest.raises(ValueError):
            apply_overrides(data, {"name": "new_name"})

    @pytest.mark.zentao("TC-S1058", domain="server/domain", priority="P2")
