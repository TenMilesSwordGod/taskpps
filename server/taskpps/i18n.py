...
_zh: dict[str, str] = {
    ...
    "Override task(s) not among retry targets: {tasks}": "覆盖的任务不在重试目标中:{tasks}",
    "Command override is not supported for task type '{type}'": "任务类型'{type}'不支持修改命令",
    "Working directory override is not supported for task type '{type}'": "任务类型'{type}'不支持修改工作目录",
